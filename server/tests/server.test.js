...
describe('POST /', () => {
    it('should return filtered JSON response', (done) => {
        ...
        request(app)
            ...
            .expect(200)
            .expect((res) => {
                expect(JSON.stringify(res.body)).toBe(JSON.stringify(expectedResult));
            })
            .end(done);
    });

    it('should not return a JSON response with invalid body data', (done) => {
        request(app)
            .post('/')
            .send({})
            .expect(400)
